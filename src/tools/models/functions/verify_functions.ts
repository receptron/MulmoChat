...
const mockContext: FunctionContext = {
    ...
    evaluateFormula: (formula: string) => {
        ...
        if (formula.startsWith("\"") && formula.endsWith("\"")) return formula.slice(1, -1);
        return formula;
    }
};

const runTest = (name: string, funcName: string, args: string[], expected: any) => {
    try {
        ...
        const isMatch = result == expected || (typeof result === 'number' && typeof expected === 'number' && Math.abs(result - expected) < 0.001);
...
const now = new Date();
const expectedSerial = (now.getTime() - new Date(Date.UTC(1899, 11, 30)).getTime()) / (24 * 60 * 60 * 1000);
...
if (todayFunc) {
    ...
    if (typeof result === "number" && Math.abs(result - Math.floor(expectedSerial)) < 2) {
