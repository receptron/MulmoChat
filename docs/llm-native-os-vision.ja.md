...
| **設定 UI 移譲** | プラグイン設定 UI をアプリに委譲 | ToolPluginConfig |
...
#### 5. 統一設定システム（Configuration System）

**現状の仕組み:**

現在の MulmoChat では以下の設定フローが実装されている:

```
localStorage ("plugin_configs_v1")
    ↓
useUserPreferences (状態管理)
    ↓
HomeView.vue (getPluginConfig 関数定義)
    ↓
useToolResults (ToolContext 構築)
    ↓
...
**現状の課題:**
- プラグインごとに設定スキーマが明示的に定義されていない
- バックエンドの設定（どの API を使うか等）がハードコードされがち
- 設定 UI の生成がプラグインごとに手動
- 設定のバリデーションが不統一
- 設定の依存関係（例: imageGen の場合は API キーが必要）が表現できない
...
interface ConfigSchema {
  ...
  ui?: {
    ...
    group?: string;  // 設定グループ（"backend", "appearance" など）
  };
...
interface ProviderConfig {
  ...
  name: string;         // 表示名
  configSchema: ConfigSchema[];  // プロバイダ固有の設定
...
interface SecretConfig {
  ...
  description?: string;
  envVar?: string;       // 環境変数名（サーバー側で設定済みの場合）
}
```
...
│  ホストアプリ層（MulmoChat）                                                  │
│                                                                             │
│  1. 全プラグインの configSchema を収集                                       │
│  2. スキーマから設定 UI を自動生成                                            │
│  3. 設定値を localStorage に永続化                                          │
│  4. context.app.getConfig() で値を提供                                      │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ↓ UI 自動生成
┌─────────────────────────────────────────────────────────────────────────────┐
...
**設計考慮:**
- 既存の getConfig/setConfig との後方互換性を維持
- カスタム UI コンポーネントもサポート（component: "custom"）
- シークレット（API キー等）は別管理（環境変数 or 暗号化ストレージ）
- 設定変更時のリアクティブな更新

---

#### 6. ツール機能宣言（Capabilities）
